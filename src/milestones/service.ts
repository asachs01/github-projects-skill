...
export class MilestoneService {
  ...
  async createMilestone(input: CreateMilestoneInput): Promise<Milestone> {
    ...
    const response = await fetch(
      `${GITHUB_REST_ENDPOINT}/repos/${owner}/${repo}/milestones`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 422) {
        throw new GitHubClientError(
          `Invalid milestone data: ${errorBody}`,
          422
        );
      }
      throw new GitHubClientError(
        `Failed to create milestone: ${response.statusText} - ${errorBody}`,
        response.status
      );
    }

    const milestoneResponse = (await response.json()) as GitHubMilestoneResponse;
  ...
  async updateMilestone(input: UpdateMilestoneInput): Promise<Milestone> {
    ...
    const response = await fetch(
      `${GITHUB_REST_ENDPOINT}/repos/${owner}/${repo}/milestones/${milestoneNumber}`,
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${this.token}`,
    ...
    if (!response.ok) {
      ...
      throw new GitHubClientError(
        `Failed to update milestone: ${response.statusText} - ${errorBody}`,
        response.status
      );
    }
  ...
  async getMilestone(owner: string, repo: string, milestoneNumber: number): Promise<Milestone> {
    ...
    if (!response.ok) {
      ...
      const errorBody = await response.text();
      throw new GitHubClientError(
        `Failed to get milestone: ${response.statusText} - ${errorBody}`,
        response.status
      );
    }
   ...
   */
  async listMilestones(input: ListMilestonesInput): Promise<Milestone[]> {
    ...
    const response = await fetch(
      `${GITHUB_REST_ENDPOINT}/repos/${owner}/${repo}/milestones?${params.toString()}`,
      {
        headers: {
          Authorization: `Bearer ${this.token}`,
          Accept: 'application/vnd.github+json',
    ...
    if (!response.ok) {
      ...
      throw new GitHubClientError(
        `Failed to list milestones: ${response.statusText} - ${errorBody}`,
        response.status
      );
    }

    const milestonesResponse = (await response.json()) as GitHubMilestoneResponse[];
  ...
  async deleteMilestone(owner: string, repo: string, milestoneNumber: number): Promise<void> {
    ...
    if (!response.ok) {
      ...
      const errorBody = await response.text();
      throw new GitHubClientError(
        `Failed to delete milestone: ${response.statusText} - ${errorBody}`,
        response.status
      );
    }
  }
   ...
   */
  async assignMilestoneToIssue(input: AssignMilestoneInput): Promise<AssignMilestoneResult> {
    const { owner, repo, issueNumber, milestoneNumber } = input;

    const response = await fetch(
      `${GITHUB_REST_ENDPOINT}/repos/${owner}/${repo}/issues/${issueNumber}`,
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${this.token}`,
    ...
    if (!response.ok) {
      ...
      if (response.status === 422) {
        throw new GitHubClientError(
          `Invalid milestone assignment: ${errorBody}`,
          422
        );
      }
      throw new GitHubClientError(
        `Failed to assign milestone: ${response.statusText} - ${errorBody}`,
        response.status
      );
    }
  ...
  async findMilestoneByTitle(
    ...
    // Exact match first
    const exactMatch = milestones.find(
    ...
    // Partial match
    const partialMatch = milestones.find(
   ...
   */
  private mapToMilestone(response: GitHubMilestoneResponse): Milestone {
 ...
 */
export function createMilestoneService(token: string): MilestoneService {
  if (!token) {
