...
describe('LinkService', () => {
  ...
  describe('linkPRToIssue', () => {
    it('links a PR to an issue by adding a comment', async () => {
      ...
      expect(result.commentUrl).toBe(
        'https://github.com/owner/repo/issues/12#issuecomment-123'
      );
    });
  ...
  describe('suggestPRLinks', () => {
    it('suggests PRs based on title matching', async () => {
      ...
      const authSuggestion = suggestions.find(
        (s) => s.issueNumber === 12 && s.suggestedPR.number === 45
      );
    ...
    it('suggests PRs based on issue number in branch name', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () =>
          Promise.resolve([createMockPR(45, 'Bug fix', 'open', 'fix/issue-12')]),
      });
    ...
    it('skips already linked PRs', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () =>
          Promise.resolve([createMockPR(45, 'Add auth', 'open', 'feature/auth')]),
      });
