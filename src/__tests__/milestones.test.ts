...
function createMockMilestoneResponse(
  number: number,
  title: string,
  ...
  html_url: string;
  url: string;
  creator: { id: number; login: string; avatar_url: string };
} {
  return {
    id: number * 1000,
...
describe('MilestoneService', () => {
  ...
  describe('updateMilestone', () => {
    it('updates a milestone', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.github.com/repos/owner/repo/milestones/1',
        expect.objectContaining({
          method: 'PATCH',
        })
      );
  ...
  describe('getMilestone', () => {
    ...
    it('throws MilestoneNotFoundError on 404', async () => {
      ...
      await expect(service.getMilestone('owner', 'repo', 999)).rejects.toThrow(
  ...
  describe('deleteMilestone', () => {
    ...
    it('throws MilestoneNotFoundError on 404', async () => {
      ...
      await expect(service.deleteMilestone('owner', 'repo', 999)).rejects.toThrow(
        MilestoneNotFoundError
      );
    });
  });
  ...
  describe('assignMilestoneToIssue', () => {
    ...
    it('throws on issue not found (404)', async () => {
      ...
      await expect(
        service.assignMilestoneToIssue({
          owner: 'owner',
          repo: 'repo',
          issueNumber: 999,
          milestoneNumber: 1,
