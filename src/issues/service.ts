...
export class IssueService {
  ...
  async createIssue(input: CreateIssueInput): Promise<Issue> {
    ...
    const response = await fetch(
      `${GITHUB_REST_ENDPOINT}/repos/${owner}/${repo}/issues`,
      {
        method: 'POST',
        headers: {
    ...
    const issueResponse = (await response.json()) as GitHubIssueResponse;
  ...
  async addIssueToProject(
    projectId: string,
    issueNodeId: string
  ): Promise<AddIssueToProjectResult> {
  ...
  async setIssueStatus(
    ...
    if (!optionId) {
      ...
      throw new GitHubClientError(
        `Status "${status}" not found in project. Available statuses: ${availableStatuses.join(', ')}`
      );
    }

    await this.githubClient.updateItemStatus(
      projectId,
      itemId,
  ...
  async createIssueInProject(
    ...
    projectNumber?: number,
    isOrg?: boolean
  ): Promise<{ issue: Issue; projectItemId: string }> {
   ...
   */
  private mapToIssue(response: GitHubIssueResponse): Issue {
    return {
      id: response.id,
 ...
 */
export function createIssueService(token: string): IssueService {
  if (!token) {
