...
import { findMatches, getSuggestions, DEFAULT_MIN_SCORE, } from '../updates/matcher.js';
...
export class CommentService {
    ...
    async addComment(input) {
        ...
        if (!response.ok) {
            ...
            if (response.status === 422) {
                throw new GitHubClientError(`Invalid comment data: ${errorBody}`, 422);
            }
            throw new GitHubClientError(`Failed to add comment: ${response.statusText} - ${errorBody}`, response.status);
        }
    ...
    async addNote(input) {
        ...
        if (matches.length > 1) {
            ...
            if (best.score - secondBest.score < AMBIGUITY_THRESHOLD && best.score < 0.9) {
                throw new AmbiguousIssueMatchError(query, matches.slice(0, 3).map(m => ({
                    title: m.title,
                    number: m.number,
                    score: m.score,
                })));
     ...
     */
    async addNoteToProjectItem(org, projectNumber, query, note, repoOwner, repoName, isOrg = true) {
        // Get project context
    ...
    async getRepoIssues(owner, repo) {
        const response = await fetch(`${GITHUB_REST_ENDPOINT}/repos/${owner}/${repo}/issues?state=all&per_page=100`, {
            headers: {
                Authorization: `Bearer ${this.token}`,
                Accept: 'application/vnd.github+json',
                'X-GitHub-Api-Version': '2022-11-28',
 ...
 */
export function createCommentService(token, options) {
    if (!token) {
